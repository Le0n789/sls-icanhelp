...
/**
 * 快捷生成带方向后缀的性能指标名称
 * labelDir('拉伸强度', 'X,Y') → '拉伸强度(X,Y 方向)'
 * 方向写法自由：'X'、'Z'、'X,Y'、'X,Y,Z'、'X/Z' 均可
 * 用法：{ label: labelDir('拉伸强度', 'X,Y'), value: 48, unit: 'MPa', testMethod: 'ISO 527-1/-2' }
 */
export function labelDir(label: string, directions: string): string {
  return directions ? `${label}(${directions} 方向)` : label;
}
...
export const MATERIALS_DATA: Material[] = [
  {
    ...
    properties: [
      ...
      { label: '负载 1.80 MPa时的挠度温度(X 方向)', value: 64, unit: '°C', testMethod: 'ISO 75-1/-2' },
      { label: '负载 1.80 MPa时的挠度温度(Z 方向)', value: 57, unit: '°C', testMethod: 'ISO 75-1/-2' },
